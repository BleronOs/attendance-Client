...
const AuthNavbar = ({
  logo, logoText, secondary, sidebarWidth, ...rest
}) => {
  ...
  const createPagesLinks = (routes) => routes.map((link) => {
    if (
  ...
  const createExtraLinks = (routes) => routes.map((link) => (
    <NavLink to={link.layout + link.path}>
      <MenuItem
        ps="36px"
        py="0px"
  ...
  const createAuthLinks = (routes) => routes.map((link) => {
  ...
  const createApplicationLinks = (routes) => routes.map((link) => (
    <NavLink to={link.layout + link.path}>
  ...
  const createEcommerceLinks = (routes) => routes.map((link) => {
    if (link.authIcon) {
      return (
        <Stack direction="column">
          <Stack
