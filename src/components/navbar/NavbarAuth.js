...
export default function AuthNavbar({
  secondary, logo, logoText, sidebarWidth, ...rest
}) {
  ...
  function getLinks(routeName) {
    const foundRoute = routes.filter((route) => route.items && route.name === routeName);
  ...
  function getLinksCollapse(routeName) {
    ...
    const foundLinks = foundRoute[0].items.filter((link) => link.collapse === true);
  ...
  const createNftsLinks = (routes) => routes.map((link, key) => (
    <NavLink
      key={key}
  ...
  const createDashboardsLinks = (routes) => routes.map((link, key) => (
    <NavLink
      key={key}
      to={link.layout + link.path}
      style={{ maxWidth: 'max-content', marginLeft: '40px' }}
  ...
  const createMainLinks = (routes) => routes.map((link, key) => {
  ...
  const createAuthLinks = (routes) => routes.map((link, key) => {
    if (link.collapse === true) {
      return (
